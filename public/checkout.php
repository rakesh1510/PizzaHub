...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if(!$error){
    ...
    db()->prepare('INSERT INTO orders(order_number,user_id,customer_name,customer_email,customer_phone,customer_address,fulfillment_type,payment_method,subtotal,delivery_fee,total,order_source,payment_status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)')
      ->execute([$num,currentUser()['id']??null,$name,$email,$phone,$addr,$ful,$pay,$subtotal,$del,$total,'online',$pay==='stripe'?'unpaid':'paid']);
    $oid=db()->lastInsertId();
    ...
    } else { db()->prepare('INSERT INTO payments(order_id,provider,amount,status) VALUES(?,?,?,?)')->execute([$oid,'cash',$total,'paid']); }
    header('Location: order_success.php?order='.$num);exit;
  }
}
?>
<h2>Checkout</h2>
...
<p class="card">For card payments, Stripe Checkout must be configured. Card numbers are not collected on this site.</p>
